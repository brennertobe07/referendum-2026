...
| `C:\DPVA_Projects\Referendum2026\analysis\analyze.py` | PY | Phase 5 | keep | Builds base tables + writes report/xlsx (incl. §5i drop-off) |
| `C:\DPVA_Projects\Referendum2026\analysis\export_dropoff_targets.py` | PY | Targeting | keep | Exports drop-off voter CSVs (PII) to an external folder — code safe to commit, output is NOT |
| `C:\Absentee\Referendum2026_Dropoff_Targets\dropoff_*.csv` | CSV (PII) | Targeting | **external, NOT in repo** | 690,881 drop-off voters (+ Dem-under-50 segment, 260,773) for VAN re-mobilization. Contains voter PII — never commit to the public repo |
...
- **Phase 5 pattern findings:** This was a **base-mobilization** electorate — 75.4% of voters had voted in 3-or-4 of the last 4 Nov generals, only 2.1% were first-time. Turnout rose steeply with age (28.7% at 18–24 → 72.0% at 65–74) and ran 1–5 pp below 2025-General levels in every band. Using the absentee-dashboard party rule (party ID, then `Dem_Support_26` split at 50), **Rep turnout 52.9% vs Dem 45.0%**, but Dem-leaning registrants outnumber Republicans ~3.7M to 2.7M and were the majority of voters (53.4% vs 46.6%), passing the amendment 51.69% Yes. Vote method: 54.9% election day / 34.7% in-person early / 10.2% mail, with 65+ skewing early/mail and under-35s skewing election day.
- **Phase 5 data quality:** LTV→RVL match is essentially perfect (only **2** of 3.1M voters unmatched). The join key `LTV.IDENTIFICATION_NUMBER = Van.StateFileID = RVL.IDENTIFICATION_NUMBER` is reliable; `Van.StateFileID` is unique (no fan-out). Van vote-history columns code voted as non-blank (A/P/Y), non-voters as NULL. Party bucketing matches the absentee/cure dashboards: `Van.LikelyParty` (SD/LD=Dem, SR/LR=Rep), then ND/U/I split by `Dem_Support_26` at 50, leaving a negligible no-score/no-VAN Unknown (a two-way Dem/Rep split, easier to read than a large Independent bucket). Covington/Sussex under-report (above) is the only material locality gap; 101 precincts exceed ±2 SD turnout but all low-end outliers are explainable (universities, Quantico, downtown cores).
- **Reproducibility:** `analysis\analyze.py` regenerates the Phase 5 body of `LTV2026_Ref_Analysis.md` + the `.xlsx`. By default it **reuses** the existing `LTV2026_Ref_Base` table (fast); set `REBUILD=1` to rebuild it from scratch (the slow LTV⋈Van⋈RVL join). The Phase 6 executive summary lives in `analysis\_exec_summary.md` and is auto-prepended to the report on every run, so regenerating no longer clobbers it — edit `_exec_summary.md` to change the summary. `analyze.py` also builds `Dropoff_2025G_Base` (reused if present; `REBUILD=1` rebuilds it, ~1 min).
- **Drop-off finding (§5i):** 20.1% of 2025-General voters (690,881) skipped the referendum — skewed young (42% of 18–24) and Democratic (22.3% vs 17.1% Rep). Re-mobilization targeting lists are produced by `export_dropoff_targets.py`.
- **PII handling:** voter-level targeting CSVs contain name/phone/address and are written to `C:\Absentee\Referendum2026_Dropoff_Targets\` — **outside** the repo. This repo is **public**; never commit individual voter files. Aggregate analysis (counts/rates) is fine to publish.
- **VAN refresh (2026-05-22):** loaded a new VAN image; the support score is now **`Dem_Support_26`** (replaces `Clarity_DemSupport_26`; 0–100, threshold 50 unchanged). Rebuilt all three base tables (`REBUILD=1`). Effect: referendum voters with **no VAN match dropped from 25,494 → 103** (the new registrants are now scored), so **surge rose to 357,726 (11.5%)** and drop-off is 682,378 (19.9%); party splits shifted marginally (Rep turnout 52.9% / Dem 45.0%; Dem 53.4% of voters). The ~25k new registrants were overwhelmingly recent (99% registered after the 2025 General) and young (47% age 18–24).
