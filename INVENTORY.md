...
- **Reproducibility:** `analysis\analyze.py` regenerates the Phase 5 body of `LTV2026_Ref_Analysis.md` + the `.xlsx`. By default it **reuses** the existing `LTV2026_Ref_Base` table (fast); set `REBUILD=1` to rebuild it from scratch (the slow LTV⋈Van⋈RVL join). The Phase 6 executive summary lives in `analysis\_exec_summary.md` and is auto-prepended to the report on every run, so regenerating no longer clobbers it — edit `_exec_summary.md` to change the summary.
